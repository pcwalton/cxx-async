// cxx-async2/examples/libunifex/src/main.rs
...
mod ffi {
    ...
    extern "Rust" {
        ...
        fn rust_libunifex_ping_pong(i: i32) -> Box<RustFutureString>;
    }

    unsafe extern "C++" {
        include!("libunifex_example.h");

        fn libunifex_dot_product() -> Box<RustFutureF64>;
        fn libunifex_call_rust_dot_product();
        fn libunifex_schedule_rust_dot_product();
        fn libunifex_not_product() -> Box<RustFutureF64>;
        fn libunifex_call_rust_not_product();
        fn libunifex_ping_pong(i: i32) -> Box<RustFutureString>;
    }
}
...
fn rust_libunifex_ping_pong(i: i32) -> Box<RustFutureString> {
    async fn go(i: i32) -> String {
        format!(
            "{}ping ",
            if i < 4 {
                ffi::libunifex_ping_pong(i + 1).await.unwrap()
            } else {
                "".to_owned()
            }
        )
...
fn main() {
    ...
    let future = ffi::libunifex_dot_product();
    ...
    ffi::libunifex_schedule_rust_dot_product();

    // Test exceptions being thrown by C++ async functions.
    let future = ffi::libunifex_not_product();
    ...
    let future = ffi::libunifex_ping_pong(0);
    println!("{}", executor::block_on(future).unwrap());
}
